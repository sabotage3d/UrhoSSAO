...
void PS()
{
    ...
    //gl_FragColor = vec4(1,0,0,1) * EncodeDepth(vTexCoord.z);

    //gl_FragColor = vec4(vsN,1) ;
    ...
    //float occlusion = position.z;

    //gl_FragColor = vec4(packedValue.x, packedValue.y, packedValue.z, 1.0);
}
